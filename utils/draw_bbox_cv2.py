...
def draw_rectange(image_np, p1, p2, color_1,score):
    cv2.rectangle(image_np, p1, p2, color_1, 3, 1)
...
def extract_box_and_plot(boxes, scores, classes,num_detections, score_thresh, im_width, im_height, image_np):
    # For Human
    color_1 = (255, 150, 0)
    bbox_coord = []
    ...
    for i in range(len(scores)):
        if scores[i]>score_thresh:
            if classes[i]==1:
                ...
                bbox_coord.append((left, right, top, bottom))
                p1 = (int(left), int(top))
                p2 = (int(right), int(bottom))
                draw_rectange(image_np, p1, p2, color_1, scores[i])
